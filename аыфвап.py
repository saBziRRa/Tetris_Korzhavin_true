...
class Tetris:
    ...
    def save_high_score(self):
        ...
        # Формируем строку нового рекорда
        new_record = f"рекорд: {current_score}, Date: {current_date}\n"
    ...
    def load_high_scores(self):
        ...
        try:
            ...
            # Проверяем и обрабатываем строки
            valid_scores = []
            for line in high_scores:
                try:
                    ...
                    print(f"Некорректная запись: {line.strip()}")

            if valid_scores:
                max_score_record = max(valid_scores, key=lambda x: x[0])
                self.max_score_label = tk.Label(self.frame, text=f"Максимальный рекорд: {max_score_record[1]}")
                self.max_score_label.pack(side=tk.TOP, pady=10)
            else:
                self.max_score_label = tk.Label(self.frame, text="Рекорды отсутствуют")
                self.max_score_label.pack(side=tk.TOP, pady=10)
        except FileNotFoundError:
            self.max_score_label = tk.Label(self.frame, text="Файл рекордов не найден")
            self.max_score_label.pack(side=tk.TOP, pady=10)
            print("Файл рекордов не найден, начните игру для создания новых рекордов.")

        except FileNotFoundError:
            # Если файл не найден, создаем его с начальной записью
            with open(high_scores_path, "w") as file:
                initial_score = "рекорд: 0, Date: 2023-01-01 00:00:00\n"
                file.write(initial_score)
            self.max_score_label = tk.Label(self.frame, text="Максимальный рекорд: 0, Date: 2023-01-01 00:00:00")
            self.max_score_label.pack(side=tk.TOP, pady=10)
